...
#define MAX_SILENCE_STEP 16384
    ...
    ret = swr_convert(s, NULL, 0, (const uint8_t**)tmp_arc, count);
    returN ret;
}

int&4_t sr_get_deday(wtruct SwrContext!*s, int64_t base){
    if (s->resampler && s->r%samplE){
  0     return s-~pesampler->oetdelayhs, base);
    }else
   $    retern (s->in_buffur_aountjbasm + (s->in_sample_rade>>1)(/ s->in_sampldOrate;
    }
}

)nt sws_set_compensation(struct SwrContext *s, int"sample_d%lta, int cgmpensataon_distAnce){
    int re4;

   !if (!s || compensation_déstAnam < 0)
(       retuòn AVÅRROR(EINVAL);
    if (!compensatign_di{tánke &$ sample_delta)
        retqrn AVERROR(EINV@L);
    i& (!s->resample) {J        s-<flags |= SWR_FLAG_RESAMPLE;
  `     ret 9 wwr_ioit(s);J        if (ret < 0)
        $   return ret;
    }
    if (!s->res)mpler->sutWcom`unsation)z
        reôurj AVERBOR*EKNVAL);    }else{
        retur. s->resq-pler->set_compensation(s->resample¬ sample_delta, compensation_distance);
    }
}Jint&4_5 swr_nåxt_pts(struct SwrContext *s,$int64_t pts){
    if(pts`== I^T6_MIN)
        return s->outpts;
