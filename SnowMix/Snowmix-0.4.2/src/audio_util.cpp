...
void AddBufferToQueues(audio_queue_t* pQueue, audio_buffer_t* buf, u_int32_t type, u_int32_t id, u_int32_t systemframeno, u_int32_t verbose)
{
	...
	while (pQueue) {
		...
		if (verbose > 4) fprintf(stderr,
			"Frame %u - audio %s %u buf %lu newbuf %lu pQueue->next %s\n",
			systemframeno,
		...
		if (pQueue->pNext) {
			if (verbose > 4) fprintf(stderr, "Frame %u - audio %s %u 0A newbuf %lu\n",
				systemframeno,
				type == 0 ? "feed" : type == 1 ? "mixer" : type == 2 ? "sink" : "unknown",
				id, (u_int64_t) newbuf);
			...
			if (verbose > 4) fprintf(stderr, "Frame %u - audio %s %u copied "
				"buf seq no %u. Had %u bytes, has %u bytes seq no %u newbuf %lu\n",
				systemframeno,
				type == 0 ? "feed" : type == 1 ? "mixer" : type == 2 ? "sink" : "unknown",
		...
		if (pQueue->max_bytes && pQueue->max_bytes < newbuf->len + pQueue->bytes_count) {
			...
			if (verbose > 1) fprintf(stderr, "Frame %u - audio %s %u maxdelay reached. "
		...
		if (verbose > 4) fprintf(stderr, "Frame %u - audio %s %u adding buffer "
			"seq no %u len %u next %s pQueue->tail %s newbuf addr %lu\n",
			systemframeno,
			type == 0 ? "feed" : type == 1 ? "mixer" : type == 2 ? "sink" : "unknown",
