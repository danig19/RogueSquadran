...
static bool NumberWithPI(const char* str, double* res)
{
	double divisor;
...
int CVideoShape::set_shape_place_coor(struct controller_type* ctr, const char* str)
{
	...
	int n = sscanf(str, "%u %lf %lf %2[nsew]", &place_id, &x, &y, &anchor[0]);
	if (n < 3) return -1;
...
int CVideoShape::set_shape_place(struct controller_type* ctr, const char* str)
{
	...
	if (*str) {
		...
		if (*str) {
			...
			while (isspace(*str)) str++;
			if (*str) {
				if (sscanf(str, "%lf %2[nsew]", &alpha, &anchor[0]) < 1) return -1;
			}
		}
	}
	...
	if (m_verbose && !n && m_pVideoMixer && m_pVideoMixer->m_pController) {
		m_pVideoMixer->m_pController->controller_write_msg (ctr,
			...
			red, green, blue, alpha, anchor[0] ? " anchor " : "", anchor);
	}
	return n;
}
...
{
	if (!m_shapes || shape_id >= m_max_shapes || !m_shapes[shape_id]) return 1;
	if (type != STROKE && type != STROKEPRESERVE && type != CLIP && type != FILL && type != FILLPRESERVE && type != CLOSEPATH && type != NEWPATH && type != SAVE && type != RESTORE && type != SCALETELL) return -1;
...
int CVideoShape::SetPlacedShapeCoor(u_int32_t place_id, double x, double y, const char* anchor)
{
	...
	if (anchor && *anchor) return SetAnchor(place_id, anchor);
	return 0;
}

int CVideoShape::SetAnchor(u_int32_t place_id, const char* anchor)
{
	if (!anchor || !m_pVideoMixer || !m_placed_shapes ||
	...
	if (strcasecmp("ne", anchor) == 0) {
		pShape->anchor_x = m_pVideoMixer->m_geometry_width;
		pShape->anchor_y = 0;
	} else if (strcasecmp("se", anchor) == 0) {
		pShape->anchor_x = m_pVideoMixer->m_geometry_width;
		pShape->anchor_y = m_pVideoMixer->m_geometry_height;
	} else if (strcasecmp("sw", anchor) == 0) {
		pShape->anchor_x = 0;
		pShape->anchor_y = m_pVideoMixer->m_geometry_height;
	} else if (strcasecmp("n", anchor) == 0) {
		pShape->anchor_x = m_pVideoMixer->m_geometry_width>>1;
		pShape->anchor_y = 0;
	} else if (strcasecmp("w", anchor) == 0) {
		pShape->anchor_x = m_pVideoMixer->m_geometry_width;
		pShape->anchor_y = m_pVideoMixer->m_geometry_height>>1;
	...
	double alpha, const char* anchor)
{
	...
	p->green = green;
	p->blue = blue;
	p->alpha = alpha < 0.0 ? 0.0 : alpha > 1.0 ? 1.0 : alpha;
	p->update = false;
	//p->anchor_x = 0;	// Set by calloc
	//p->anchor_y = 0;	// Set by calloc
	if (anchor && *anchor) {
		int n = SetAnchor(place_id, anchor);
		if (n) {
			free(p);
			return -1;
		}
...
// shape overlay (<id> | <id>..<id> | <id>..end | all) [(<id> | <id>..<id> | <id>..end | all)] ....
...
int CVideoShape::list_shape_help(struct controller_type* ctr, const char* str)
{
	if (m_pVideoMixer && m_pVideoMixer->m_pController) {
		...
		m_pVideoMixer->m_pController->controller_write_msg (ctr,
			...
			MESSAGE"shape overlay <place id> (<place id> | <place id>..<place id> | <place id>..end) ...\n"
...
int CVideoShape::list_shape_place_help(struct controller_type* ctr, const char* str)
{
	if (m_pVideoMixer && m_pVideoMixer->m_pController) {
		m_pVideoMixer->m_pController->controller_write_msg (ctr, MESSAGE"Commands:\n"
			...
			MESSAGE"shape place alpha [<place id> <shape id> <alpha>]\n"
			MESSAGE"shape place coor [<place id> <x> <y> [ nw | ne | se | sw | n | s | e | w ]]\n"
